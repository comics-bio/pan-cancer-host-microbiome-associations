...
short_row_names_2 <- substr(rownames(cell), 1, 15)
cell <- cell[!duplicated(short_row_names_2), ]  # remove duplication randomly
rownames(cell) <- substr(rownames(cell), 1, 15)
...
write.table(pair_wise_res, paste0(project_code, '.spear.correlation.test.result.tsv'), sep='\t', row.names = F, quote=F)
